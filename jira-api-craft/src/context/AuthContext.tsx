...
interface AuthContextType {
  ...
  loginJira: (user: User, token: string) => void;
  logoutJira: () => void;
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [jiraAccessToken, setJiraAccessToken] = useState<string | null>(() => {
    try {
      ...
      const exp = localStorage.getItem("jira_token_exp");
      if (token && exp && Number(exp) > Date.now()) return token;
      // expired or missing
      try { localStorage.removeItem('jira_token'); localStorage.removeItem('jira_token_exp'); } catch (e) {/* ignore */ }
      return null;
    } catch (e) {
      return null;
    }
  ...
  const [jiraTokenExp, setJiraTokenExp] = useState<number | null>(() => {
    try {
      if (typeof window === 'undefined') return null;
      const s = localStorage.getItem('jira_token_exp');
  ...
  const loginJira = useCallback((user: User, token: string, expiresInSeconds: number = 3600) => {
  ...
  return (
    <AuthContext.Provider value={{
      ...
      jiraUser,
      jiraAccessToken,
      isJiraAuthenticated: jiraUser != null && jiraAccessToken != null && (jiraTokenExp ? jiraTokenExp > Date.now() : true),
      loginJira,
      logoutJira,
